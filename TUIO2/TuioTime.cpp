...
TuioTime::TuioTime(osc::TimeTag timetag) {
	...
	micro_seconds = static_cast<long>(frac / NTP_UNITS);
}
...
osc::TimeTag TuioTime::getSystemTimeTag() {
	...
	osc::uint32 secs = static_cast<osc::uint32>((osc::uint32)systemTime.getSeconds() + JAN_1970);
	osc::uint32 frac = static_cast<osc::uint32>(systemTime.getMicroseconds() * NTP_UNITS);
